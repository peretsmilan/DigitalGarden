    ...
    if (preloader) {
        setTimeout(() => {
            ...
            if (manimVideo) {
                ...
                manimVideo.play().catch(err => console.log("Video autoplay caught by browser policy:", err));
            }
...
const softwareData = {
    vscode: {
        ...
        extensions: [
            { name: "Prettier - Code Formatter", desc: "Enforces a consistent style by parsing your code and re-printing it with its own rules, reducing manual layout formatting effort completely." },
            { name: "Error Lens", desc: "Improves language diagnostics features by making diagnostics stand out more prominently, highlighting the entire line and printing messages inline." },
            { name: "Todo Tree", desc: "Searches your workspace for comment tags like TODO and FIXME, then displays them beautifully organized in a tree view container in the activity bar." },
            { name: "Best Themes Redefined", desc: "Soothing pastel high-contrast, low-brightness theme designed explicitly to diminish eye strain during extended late-night coding intervals. Specifically, 'One Monokai Darker'" },
            { name: "Custom Fonts", desc: "Use custom fonts like Fira Code, JetBrains Mono, and more to enchance readability and aesthetics with features like ligatures and improved character spacing." }
        ],
    ...
    obs: {
        ...
        extensions: [
            { name: "Hardware Encoding (NVENC / AMF)", desc: "Using NVIDIA NVENC or AMD HW H.264/H.265 offloads encoding tasks entirely to your GPU's dedicated silicon chip, maximizing game performance and avoiding encoding overloads." },
            { name: "Software Encoding (x264)", desc: "Utilizes your system CPU for video encoding. While it can offer crisp detail at lower bitrates, it incurs a massive processor tax that can cause heavy frame drops in resource-intensive tasks." },
            { name: "Safe Recording Formats (.mkv)", desc: "Always set your output to record in .mkv. Unlike .mp4, an .mkv file preserves your data up to the exact millisecond of a crash or power outage, preventing the entire video from corrupting." },
            { name: "Automatically Remux to .mp4", desc: "Enable 'Automatically remux to mp4' in Advanced settings. OBS will instantly clone your safe .mkv file into a universally compatible .mp4 format the second your recording stops cleanly." },
            { name: "CQP (Constant Rate Factor)", desc: "The ideal rate control for local recordings. Set your CQP level between 18 and 23 to dynamically allocate bitrate based on visual complexity rather than wasting space on a static target." }
        ],
    ...
    powertoys: {
        ...
        extensions: [
            { name: "1. Download and Install", desc: "Visit the official Microsoft PowerToys page, link below, and download the latest release. Run the installer and follow the prompts to correctly set everything up!" },
            { name: "2. Explore the Utilities", desc: "Manage your windows with FancyZones, manage Input & Outputs, play around with System Tools, File Managment, and Advanced Tools, these include lots of utilities that are easy to use!" },
            { name: "3. Customize Your Experience", desc: "Customize your Windows! Using Command Palette to easily find your files, launch apps, and do so much more with the most extensible launcher and dock." }
        ],
    ...
    virtualbox: {
        ...
        extensions: [
            { name: "Snapshot System", desc: "Save the absolute exact state of a virtual desktop layout. If an optimization brick file ruins the engine, roll it back to your secure snapshot in seconds." },
            { name: "Guest Additions", desc: "A suite of drivers installed inside the virtual OS providing seamless mouse integration, shared clipboards, drag-and-drop file support, and native monitor resolutions." },
            { name: "Virtual Media Manager", desc: "Dynamically allocate storage parameters so your secondary host machine only scales on your storage drives as it populates internal content layers." }
        ],
...
const tutorialData = {
    ligatures: {
        ...
        steps: [
            { name: "1. Download a Compatible Font", desc: "Download and install a font that supports ligatures, such as <strong>JetBrains Mono</strong> or <strong>Fira Code</strong> onto your operating system." },
            { name: "2. Open VSCode Settings JSON", desc: "Press <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>P</kbd> (or <kbd>Cmd</kbd> on Mac), type 'Open User Settings (JSON)', and select it." },
            { name: "3. Update Your Configuration", desc: "Add or update these lines inside your settings object:<br><code>\"editor.fontFamily\": \"'JetBrains Mono', monospace\",</code><br><code>\"editor.fontLigatures\": true,</code>" },
            { name: "4. Restart Your Editor", desc: "Save the file and restart VSCode to see your new character symbols update instantly." }
        ]
    },
    layouts: {
        ...
        steps: [
            { name: "1. Set Video Resolution", desc: "Navigate to Settings > Video. Change your Base (Canvas) Resolution to <strong>1080x1920</strong> to lock in a true 9:16 portrait viewport." },
            { name: "2. Adjust Output Scaling", desc: "Set your Output (Scaled) Resolution matching your Base, or downscale smoothly to 720x1280 depending on device constraints." },
            { name: "3. Align Capture Sources", desc: "Stretch game captures using the transform tool to isolate horizontal focal elements into vertical layouts seamlessly." }
        ]
    },
    bitrates: {
        ...
        steps: [
            { name: "1. Locate Bitrate Settings", desc: "Go to Settings > Output and select the 'Recording' Tab. Switch Output Mode to 'Advanced'." },
            { name: "2. Implement CQP Controls", desc: "Set Rate Control properties directly to CQP instead of the standard streaming standard CBR parameters." },
            { name: "3. Establish Quality Scaling Indexes", desc: "Map your CQ Level accurately. Standard benchmarks run safely between 18 (near lossless, large files) and 23 (efficient scaling balance)." }
        ]
    }
};
...
routes.forEach(route => {
    route.addEventListener("click", (e) => {
        ...
        document.querySelectorAll(".nav-route").forEach(link => link.classList.remove("active"));
        if (route.classList.contains("nav-route")) {
            route.classList.add("active");
        } else {
            document.querySelector('.nav-route[data-target="home-page"]').classList.add("active");
        }
        ...
        if (data) {
            ...
            modalBody.innerHTML = `
                ...
                <div class="modal-actions">
                    <a href="${data.downloadUrl}" target="_blank" class="primary-btn">Download Tool &rarr;</a>
                </div>
            `;
            openModal();
        }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function playClickSound() {
        if (audioToggle && audioToggle.checked) {
            ...
            clickSound.play().catch(err => console.log("Audio feedback waiting on gesture."));
        }
    }
    ...
    function playSwooshSound() {
        if (audioToggle && audioToggle.checked) {
            ...
            swooshSound.play().catch(err => console.log("Audio feedback waiting on gesture."));
        }
    }

    const clickElements = document.querySelectorAll(".software-card, .social-card, .tutorial-card, button, select, #settings-trigger-btn, .close-sidebar-btn, .real-checkbox");
    clickElements.forEach(element => {
        element.addEventListener("click", () => {
            if (element.id !== "ghost-toggle-checkbox") {
                playClickSound();
            }
        });
    });

    const navRoutes = document.querySelectorAll(".nav-route, .nav-logo");
    ...
    // DISPLAY NAME WITH TIMEOUT & SAVING
    ...
    if (displayNameInput && applyPrompt && saveNameBtn) {
        ...
        setTimeout(() => {
            ...
            if (savedName) {
                ...
                // Force custom display name on load if found
                if (emailDisplay) emailDisplay.textContent = savedName;
            } else {
                const userEmail = localStorage.getItem("userEmail") || (emailDisplay ? emailDisplay.textContent : "");
                
                if (userEmail && userEmail !== "—") {
                    confirmedName = userEmail.split("@")[0]; // splits text123@gmail.com -> text123
        ...
        }, 500); // 500ms allows Firebase DOM string injections to populate first

        // Handle user input text updates
        displayNameInput.addEventListener("input", function () {
            clearTimeout(typingTimer);
            applyPrompt.classList.remove("show");

            // Only prepare change confirmation if current input is distinct from stored copy
        ...
        // Click handler to commit value settings
        saveNameBtn.addEventListener("click", () => {
            ...
            if (newName !== "") {
                ...
                // Dynamically change the structural heading block text layout inside settings
                if (emailDisplay) {
                    emailDisplay.textContent = newName;
                }
    ...
    if (spaceBox) {
        ...
        spaceBox.addEventListener("change", function () {
            playClickSound();
            const currentMode = this.checked ? "tight" : "normal";
            document.documentElement.setAttribute("data-space", currentMode);
            localStorage.setItem("garden-space-mode", currentMode);
        });
    }
    ...
    if (languageSelect) {
        languageSelect.value = localStorage.getItem("garden-language") || "en";
